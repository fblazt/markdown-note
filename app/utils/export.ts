...
/**
 * Escapes string for safe HTML injection.
 */
...
export function exportNotePlainText(note: Note): string {
  ...
  let text = content
    // Remove fenced code blocks, retaining inner content
    .replace(/```[\w-]*\n([\s\S]*?)\n```/g, '$1')
    ...
    .replace(/(\*|_)(.*?)\1/g, '$2')
    .replace(/~~(.*?)~~/g, '$1')
    // Convert task checkboxes - [x] or - [ ] -> [x] / [ ]
    .replace(/^(\s*)[*+-]\s+\[([ xX])\]\s+/gm, '$1[$2] ')
    // Convert list bullets
    .replace(/^(\s*)[*+-]\s+/gm, '$1- ')
    // Remove table divider lines |---|---|
    .replace(/^\s*\|?[\s\-:|]+\|\s*$/gm, '')
    // Clean table pipe characters
...
/**
 * Exports note(s) as pretty-printed JSON.
 */
...
/**
 * Triggers a browser download of the given content string as a Blob file.
 */
...
/**
 * Prints a note using a temporary hidden iframe with standalone styled HTML.
 */
export function printNote(note: Note): void {
  if (typeof window === 'undefined') return;

  const htmlContent = exportNoteHtml(note, { standalone: true });
